...
type GMTextCoreVariant = "title" | "subtitle" | "body" | "secondary" | "section_title";
...
const TITLE_CASE_VARIANTS: GMTextVariant[] = ["title", "subtitle"];
...
const VARIANT_STYLE: Record<GMTextCoreVariant | "stat" | "code", VariantStyle> = {
  ...
  stat: { fz: "2rem", fw: 700, color: PRIMARY, lh: 1.1, letterSpacing: "-0.03em" },
...
// Applies title case to Latin text; Chinese/CJK characters are unaffected.
...
function renderHighlight(text: string, highlight: string): React.ReactNode {
  ...
  return (
    <>
      {before}
      <span style={{ color: "var(--mantine-color-brand-6)", fontWeight: 700 }}>{highlight}</span>
      {after}
    </>
  );
}
 ...
 * title + subtitle auto-apply toTitleCase.
 */
function GMText({
  variant = "body",
  children,
  ...
  const text = TITLE_CASE_VARIANTS.includes(variant) ? toTitleCase(children) : children;
  ...
  if (variant === "badge") {
    ...
    return (
      ...
      >
        {label && <span style={{ color: MUTED, fontWeight: 400 }}>{label} ·</span>}
        <span>{text}</span>
      </span>
    );
  }
  ...
  if (variant === "code") {
    ...
    const codeStyle = { fontSize: fz, fontWeight: fw, color: resolvedCodeColor };
    return inline ? (
      <Code style={codeStyle}>{text}</Code>
    ) : (
      <Code block style={{ ...codeStyle, border: "1px solid var(--mantine-color-default-border)" }}>
        {text}
      </Code>
    );
  }
