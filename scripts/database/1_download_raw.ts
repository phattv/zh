/**
 * Step 1 — Download raw HSK 1–6 word lists from glxxyz/hskhsk.com.
 *
 * Output: database/data/1_hsk{1..6}.txt
 */
...
const FILES = [1, 2, 3, 4, 5, 6].map((level) => ({
  ...
  dest: path.join(dataDir, `1_hsk${level}.txt`),
}));
...
console.log("Next: npx tsx scripts/database/2_concat_base.ts");
